    ...
    for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        ...
        with torch.no_grad():
            ...
            if net.n_classes == 1:
                ...
                #ignoring background
                dice_softmax_nobg += multiclass_dice_coeff(mask_pred_softmax[:,1:,...], mask_true[:,1:,...], reduce_batch_first=True)
            ...
            num_correct+=(mask_pred_onehot[:,1:,...]==mask_true[:,1:,...]).sum() # compute the acc score, ignoring background
            num_pixels+=torch.numel(mask_pred[:,1:,...])
           

    net.train()
    return dice_softmax_nobg / num_val_batches,dice_softmax_bg / num_val_batches,dice_onehot_nobg / num_val_batches,dice_onehot_bg / num_val_batches, num_correct/num_pixels
